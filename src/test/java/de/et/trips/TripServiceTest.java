...
public class TripServiceTest {
    ...
    TripService service = new TripService(new GroupFilterDriverVehicle());

    @Test
    public void testGroupTripsByDriverAndVehicle() {
        ...
        assertThat(group2.getTrips(), contains(trip3));
        assertThat(group2.getGroupProperty().toString(), is("GroupProperty(Lea, Truck02)"));
    }

    @Test
    public void testGroupTripsByDepartureLocation() {
        service.setGroupFilter(new GroupFilterLocation());
        List<Trip> trips = new ArrayList<>();
        DateTimeFormatter df = DateTimeFormatter.ofPattern("dd.MM.yyyy HH:mm");
        Trip trip1 = new Trip("1", "John", "Truck01", "Detroit", LocalDateTime.parse("12.12.2015 08:20", df));
        Trip trip2 = new Trip("2", "John", "Truck01", "Atlanta", LocalDateTime.parse("13.12.2015 09:13", df));
        ...
        assertThat(groups, is(iterableWithSize(2)));
        TripGroup group = groups.get(0);
        assertThat(group.getTrips(), contains(trip1));
        assertThat(group.getGroupProperty().toString(), is("GroupProperty(Detroit)"));

        TripGroup group2 = groups.get(1);
        assertThat(group2.getTrips(), contains(trip2, trip3));
